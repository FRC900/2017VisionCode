...
int main(int argc, char **argv)
{
    ...
    if (argc < 2)
        cerr << "mexopencv:error Wrong number of arguments";
	...
	pyrDown(src, src);
    Mat dst = doWork(src, use_lab, remove_border, use_geodesic);
	pyrDown(src, src);
	pyrDown(src, src);
	imshow("SRC", src);
	...
	imshow("DST", dst);
	Mat thresh;
	dst.convertTo(thresh, CV_8UC1, 255.);
	imshow("THRESH Pre", thresh);

	threshold(thresh, thresh, 0, 255, CV_THRESH_BINARY|CV_THRESH_OTSU);
	Mat el = getStructuringElement(MORPH_ELLIPSE, Size(5,5));
	erode(thresh, thresh, el, Point(-1, -1), 1);
	dilate(thresh, thresh, el, Point(-1, -1), 1);
